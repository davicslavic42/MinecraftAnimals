...
namespace MinecraftAnimals
{
	public class MCAWorld : ModWorld
	{
		...
		private void Treebases(GenerationProgress progress)
		{
			...
			for (int k = 0; k < (int)((Main.maxTilesX * (WorldGen.worldSurface * 0.20)* (WorldGen.worldSurfaceLow + 50)) * 1E-01); k++)
			{
				// The inside of this for loop corresponds to one single splotch of our Ore.
				// First, we randomly choose any coordinate in the world by choosing a random x and y value.
				int x = WorldGen.genRand.Next(0, Main.maxTilesX);
				int y = WorldGen.genRand.Next((int)(WorldGen.worldSurface * 0.20), (int)(WorldGen.worldSurfaceLow + 50)); // WorldGen.worldSurfaceLow is actually the highest surface tile. In practice you might want to use WorldGen.rockLayer or other WorldGen values.
